...
def test_ppo_optimization_loop():
    ...
    extractor_module = TensorDictModule(
        extractor, in_keys=["param"], out_keys=["loc", "scale"]
    )
    ...
    value_module = ValueOperator(
        val_net, in_keys=["observation"], out_keys=["state_value"]
    )
    ...
    optimizer = torch.optim.SGD(
        list(policy_model.parameters()) + list(value_module.parameters()), lr=0.1
    )
    ...
    for _ in range(ppo_epochs):
        ...
        adapted_val_params = OrderedDict(
            (k, v) for k, v in adapted_val_params_list.items()
        )
